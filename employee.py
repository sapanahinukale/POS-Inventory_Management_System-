...
class employeeClass:
    def __init__(self,root):
        ...
        #img1=Image.open('images/e1.png')
        #img1=img1.resize((300,100),Image.ANTIALIAS)
        #self.leftimg=ImageTk.PhotoImage(img1)
        #lblimg1=Label(self.root,image=self.leftimg,bd=2,relief=RIDGE)
        #lblimg1.place(x=0,y=0,width=300,height=100)
        
        #img2=Image.open('images/e2.png')
        #img2=img2.resize((300,100),Image.ANTIALIAS)
        #self.rightimg=ImageTk.PhotoImage(img1)
        #lblimg2=Label(self.root,image=self.rightimg,bd=2,relief=RIDGE)
        #lblimg2.place(x=800,y=0,width=300,height=100)
        
        self.bg=ImageTk.PhotoImage(file='images/product1.png')
        ...
        validate_contact=self.root.register(self.isnumber)
        ...
        validate_email=self.root.register(self.checkname)
        ...
        txt_pass=Entry(self.root,textvariable=self.var_pass,font=("goudy old style",15),show='*',bg="lightyellow").place(x=500,y=230,width=180)
...
#=====================================================================
    ...
    def add(self):
        ...
        cur=con.cursor()
        try:
            if self.var_emp_id.get()=="" or self.var_name.get()=="" or self.var_doj.get()=="" :
    ...
    def checkemail(self,email):
        if len(email)>7:
            if re.match("^([a-zA-Z0-9]_\-\.]+)@([a-zA-Z0-9_\-\.]+)\.([a-zA-Z]{2,5})$",email):
                return True
            else:
                messagebox.showerror('Alert','invalid email enter valid user email')
                return False
        else:
            messagebox.showerror('Invalid','Email length is too samll')
